...
-- TABLA DE RESIDENTES AUTORIZADOS (WHITELIST)
CREATE TABLE IF NOT EXISTS residents_whitelist (
...
ALTER TABLE residents_whitelist ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can manage whitelist" ON residents_whitelist
    USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

CREATE POLICY "Anyone can check if email is whitelisted" ON residents_whitelist
