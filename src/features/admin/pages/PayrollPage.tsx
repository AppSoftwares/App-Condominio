...
export const PayrollPage: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    const tableHeaders = ["Empleado", "Cédula", "Cargo", "Sueldo USD", "Asignaciones (Bs)", "Deducciones (Bs)", "Neto a Pagar (Bs)"];
    const rows = employees.map(emp => {
      const calc = calculatePayroll(emp);
      return [emp.nombre, emp.cedula, emp.cargo, emp.sueldo_usd, calc.totalAsignaciones.toFixed(2), calc.totalDeducciones.toFixed(2), calc.neto.toFixed(2)];
    });
    const wsData = [...header, tableHeaders, ...rows];
    const ws = XLSX.utils.aoa_to_sheet(wsData);
    ws['!cols'] = [{ wch: 25 }, { wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 18 }, { wch: 18 }, { wch: 18 }];
